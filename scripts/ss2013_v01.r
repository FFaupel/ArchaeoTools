...
arbverz <- "/home/fon/daten/lehre/ss2013/mod/modproj"  # variable with the path
...
crs1 <- "+proj=tmerc +lat_0=0 +lon_0=9 +k=1 +x_0=3500000 
    +y_0=0 +ellps=WGS84 +units=m +no_defs" # projected gk3 (command in two lines)
...
library("KernSmooth")
ks_vil <- bkde(df_vil[,3], kernel="normal", bandwidth=5, gridsize=50)
plot(ks_vil,col="black", pch=16)
lines(ks_vil)

plots(df_vil[,3],df_vil[,1],col="black", pch=16)
...
for(i in seq(along=fsg@coords[,1])) {         
    ...
    for(k in seq(along=(fsnn[i][[1]]))) {   
        pid2 <- fsnn[[i]][k]    
        if (pid2 > pid1) {     
            x2 <- fsg@coords[pid2,1]; y2 <- fsg@coords[pid2,2] 
